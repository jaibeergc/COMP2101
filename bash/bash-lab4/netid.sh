...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    ;;
    * )
      if [ "$customInterfaceName" == "" ]; then
        customInterfaceName="$1"
      fi
    ;;
  esac
  shift
...
# Task 2:  Report of Every Interface
# Getting list of allInterfaces
allInterfaces=`ifconfig | awk '{print $1;}'| sed '/^$/d;2,8d;11,17d'`
# I tried this command too but it didn't work for all existing interfaces
# allInterfaces=$(netstat -i | awk 'NR > 2 { print $1}')
# Printing report for each interface in allInterfaces
for interface in $allInterfaces; do
  # Ignoring lo: interface
